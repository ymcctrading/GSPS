...
| **Finnhub** (analyst rating + price target, ticker Company tab) | Free | 60 requests/min, no card required | Calls beyond the limit 429; `lib/data/finnhub.ts` treats any failure (missing key, 429, network error, timeout) as "no data" and falls back to the tab's simulated analyst rating/price target for that request — never a broken page. | `FINNHUB_API_KEY` required; unset = fully simulated. Two calls per Company-tab load (recommendation trends + price target) — cheap at current traffic, but re-check this row if the tab gets a hot polling loop later. Short interest and institutional/13F ownership are **not** on Finnhub's free tier and stay simulated regardless of this key. |
