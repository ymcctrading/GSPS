...
|---|---|---|---|---|
| **Vercel** | Hobby (free) | 2 cron jobs per project, each ≤ once/day. Deployments/builds also capped (see Vercel dashboard for current usage). | Deploy fails, or the cron silently isn't created. | `vercel.json` currently defines **1** cron (`/api/market-scan`, 17:30 ET weekdays) — **1 of 2 slots is free.** The 08:30 ET run of the same route moved to GitHub Actions (`.github/workflows/premarket-scan.yml`) to free that slot; see below. `/api/intraday-scan` needs to run every ~15 minutes during the session, well past the 1-run/day cap, so it is scheduled the same way: `.github/workflows/intraday-scan.yml` calls it with `CRON_SECRET`, outside `vercel.json` entirely. The route still also serves on-demand requests from a signed-in user's Scanner tab, same as before — that path is unauthenticated-by-cookie and never touches email notifications. Git-triggered deploys are **on**: a branch push builds a preview and a merge to `main` releases to production — see `AGENTS.md`. |
