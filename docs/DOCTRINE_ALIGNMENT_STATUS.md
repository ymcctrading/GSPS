...
| **Formal, single versioned "canonical decision record"** spanning Signal Context → Structure Zone → Trade Map → Order/Trade → Learning Record as one explicit contract with a shared record-version field. | The design pass this row used to call for is done — see `docs/CANONICAL_DECISION_RECORD_DESIGN.md` (this pass): `scan_results.id`/`daily_scans.id` stay the canonical id (no sixth schema generation), gaining `scan_execution_id`, `model_version`, `session`, `timeframe`, `data_freshness_status`, `pivot_plan`, and `record_schema_version` columns. That design also surfaces a fourth, previously undocumented schema generation (`scan_events`/`execution_events` from migration `0005`) that is currently unwired — nothing in the live scan or order path calls it. The migration itself is *not* written yet — schema, backfill-free write-path wiring, and the Learning Brain event log's retire-or-wire decision are still open. | Next: Migration PR implementing the reviewed design, then the write-path PR(s) it specifies. Not applied to production in this pass — see `AGENTS.md` on migration/production-change confirmation. |
