-- 0022/0023 shipped six `security definer` functions that take `user_id` as
-- a plain parameter and never check it against the caller — Supabase exposes
-- every function in the `public` schema as a PostgREST RPC by default
-- (/rest/v1/rpc/<name>), so any signed-in user could call
-- get_performance_metrics with someone else's UUID and read their trade P&L,
-- or call is_in_quiet_hours/get_enabled_notification_channels to probe another
-- user's notification settings. Caught by `get_advisors` (security) right
-- after 0022/0023 were first applied to this project.
--
-- The app's own call sites (app/api/portfolio/analytics/route.ts,
-- app/api/intraday-scan/route.ts) go through the *service role* client,
-- having already verified the caller's identity themselves — service_role
-- has no `auth.uid()` (no user JWT), so the guard below has to recognize and
-- exempt it explicitly rather than just checking auth.uid() = user_id.
--
-- Also sets `search_path` on all six, closing the separate (lower-severity)
-- search_path-hijack warning the same advisor run raised for each.
